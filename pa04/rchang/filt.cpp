...
void filtImage(unsigned char * image,double ** kernel){ //use kernel to convolve image
	...
	for(int i = 0 ;i < yres ; i++){
		for(int j = 0 ; j< xres ; j++){
			for(int k = 0 ; k < channels ; k ++){
				tempPixels[i*xres*channels + j*channels +k] = image[i*xres*channels+ j*channels +k];
			}
		}
	}
...
void resetDisplayPixels(){
	displayPixels = new unsigned char[xres*yres*channels];
		for(int i = 0 ;i < yres ; i++){
		for(int j = 0 ; j< xres ; j++){
			for(int k = 0 ; k < channels ; k ++){
				displayPixels[i*xres*channels + j*channels +k] = oPixels[i*xres*channels+ j*channels +k];
			}
		}
	}
